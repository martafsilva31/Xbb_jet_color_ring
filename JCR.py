...
sample_name = "test" #Change here the dataframe 

subjets_df = pd.read_hdf('/lstore/calo/martafsilva/Xbb/Jet_color_ring/Mar28_analysis/SamplesMar28_dataframes/%s.h5'%sample_name, key="table")
...
#subjets_df_JCR = subjets_df.dropna().reset_index(drop=True)


# Apply the function to each row of the DataFrame
subjets_df["jet_color_ring"] = subjets_df.apply(jet_color_ring, axis=1)


#Defining Jet color ring variant 

def jet_color_ring_l(row):
...
# Apply the function to each row of the DataFrame
subjets_df["jet_color_ring_l"] = subjets_df.apply(jet_color_ring_l, axis=1)
...
subjets_df.to_hdf('df_JCR_%s.h5'%sample_name, key='table', mode='w')
